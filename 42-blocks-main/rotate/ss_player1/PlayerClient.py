...
def return_ans(i,j,block_rot,b):
    J = hex(j)[2:].upper()
    I = hex(i)[2:].upper()
    block = block_rot
    ret = block_rot+J+I
...
def get_ans(l,block_list):
    ...
    for lli in range(1,5):
        if lli==1:# corner is left up
            ys = -1
            xs = -1
        if lli==2:# corner is right up
            ys = -1
            xs = +1
        if lli==3:
            ys = +1
                ...
                for block in d.keys():
                    ...
                    if(1<=(i-(ys)*Y_SIZE)<=15)and(1<=(j-(xs)*X_SIZE)<=15):
                        ...
                        for y in range(Y_SIZE):
                            if break_flg == 1:
                                break
                            for x in range(X_SIZE):#kaburuka
                                if l[((i+y+y1)<<4)+j+x+x1]!=0:
                                    break_flg = 1
                                    cnt=1
                                    break
                                if break_flg == 1:
                            ...
                            if cnt==0:
                                if lli==1:
                                    return return_ans(i,j,block,b_name)
                                else:
                                    for luy in range(Y_SIZE):
                                        for lux in range(X_SIZE):
                                            d[block][luy][lux]==1
                                            break_flg = 1
                                            break
                                        break
